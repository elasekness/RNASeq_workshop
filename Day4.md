...
> Unfortunately, this tool does not allow us to convert between locus tags and KO identifiers (K numbers) of KEGG. <br>
...
> In this case, our graph is directed, with the arrows showing the direction of the interaction. <br>


# Resize vertices based on their connectedness (the number of edges connecting them to other vertices).
...
> To make this graph more informative, we can resize and color the graph's attributes (i.e. nodes and edges) by data from our DESeq2 and enrichment analyses. <br>
...
First ensure that our gene names from our DESeq2 results match the vertex names. In this case, the vertex names are displayed as the locus tag, proceeded by `pau:`. 
...
> We overwrite the vertex names of our `igraph_object` object, searching for instances of `pau:` and replacing the expression with nothing or `""`. <br>
