...
**Note:** `Salmon` (the read quantification program we'lll use) is designed so that read trimming is not actually necessary (i.e. you can use raw reads). But we'll practice here anyway.
