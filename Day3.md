...
We will load the data from the txi object and the `ColData` dataframe into the DESEqDataSet object.  
...
The normalization factor or size factor for each sample is the median of that sample's ratios, which should minimize the influence of differentially expressed genes.  Normalized counts are generated by dividing raw counts of a sample
by its size factor.
...
PCA (Principal Component Analysis) reduces multi-dimensional data into lower dimensional space to highlight the most important sources of variation - represented by our principle components.  The first
two principal components (which explain the most variation in our data) are plotted against each other for each sample. Samples will appear close together in the plot if they share similar expression levels 
...
> Now, we are showing the variation among samples as well as the impact of the most influential genes on the first two principle components with the `showLoadings=TRUE` option. <br>
...
> This helps use to see which genes are contributing to the most variation in our data. <br>
