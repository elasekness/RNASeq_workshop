...
Now create a design table that specifies the condition(s) associated with each file.  Ensure that the file names are listed in the same order as they were imported. This file will tell DESeq2 how the data should be analyzed.
...
> Setting the **`blind`** option to **`TRUE`** ensures that the calculation is not influenced by sample information.
